...
Detecting non-IID distribution shift across hospitals ahead of federated learning, using persistence diagrams as a privacy-preserving alternative to raw patient data. This project benchmarks four TDA-based two-sample hypothesis tests against a raw-covariate baseline, on both synthetic hospital data and real TCGA cancer data.
...
- **Persistence landscape permutation test** (Bubenik): a permutation test on the integrated squared distance between groups' mean persistence landscapes.
- **Persistence landscape z-test** (Bubenik): a two-sample z-test on total landscape mass (area under the landscape).
...
| `TDA_TCGA_data_experiments_and_assumption_checks.ipynb` | Applies the R&T permutation test, landscape permutation test, and raw-covariate Wasserstein baseline to real TCGA patient data across the 6 largest contributing hospitals (omnibus, 6-way test). Covariates are combined via a Gower distance matrix to handle the mix of continuous and categorical patient features, and includes outlier detection/removal and assumption checks (fourth-moment convergence) on the real data. |
